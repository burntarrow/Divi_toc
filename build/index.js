(() => {
  ...
  const collectHeadings = (container, levels, ignore, used) => {
  ...
  const renderList = (items, nested, settings, depth = 0) => {
    const list = document.createElement('ul');
    list.className = depth === 0 ? 'divi-toc-list' : 'divi-toc-sub-list';
    if (settings?.design?.listStyle) {
      list.style.listStyleType = settings.design.listStyle;
    }
    ...
    items.forEach((item) => {
      ...
      if (settings?.backToTop?.enabled && settings.backToTop.mode === 'section') {
        ...
        top.addEventListener('click', (event) => {
          ...
          smoothScroll(topAnchor, settings.scrollOffset || 0);
        });
        li.appendChild(top);
      }
  ...
  const activateScrollspy = (nav, headings, offset) => {
    const map = new Map();
    nav.querySelectorAll('.divi-toc-item').forEach((item) => {
      const link = item.querySelector('a');
      if (link?.hash) {
        map.set(link.hash.replace('#', ''), item);
      }
    });
  ...
  const createDropdown = (items, select, depth = 0) => {
    items.forEach((item) => {
  ...
  const applyDesignStyles = (nav, settings) => {
  ...
  const ensureTopAnchor = (container) => {
  ...
  const initSingle = (nav, settings) => {
    ...
    const headingEls = headings.map((item) => item.element).filter(Boolean);
    ...
    if (settings?.backToTop?.enabled && settings.backToTop.mode === 'floating') {
      ...
      btn.addEventListener('click', (event) => {
        ...
        smoothScroll(anchor, settings.scrollOffset || 0);
      });
  ...
  const initToc = () => {
    const nodes = Array.from(document.querySelectorAll('[data-divi-toc]'));
    if (!nodes.length) return;
    if (nodes.length > 1) {
      console.warn('[Divi TOC] Multiple TOC modules detected; only the first will be initialized.');
    }
    const nav = nodes[0];
    const data = JSON.parse(nav.dataset.diviToc || '{}');
    initSingle(nav, data);
  };

  document.addEventListener('DOMContentLoaded', initToc);
  window.diviTOCInit = initToc;
})();
